...
export class CourseRepository extends BaseRepository<ICourse> implements ICourseRepository {
  ...
  async findAll(): Promise<ICourse[]> {
    const courses = await this.model.find({}).populate("instructor","name email")
    return courses;
  }

  async findCourseById(courseId: string): Promise<ICourse | null> {
    const course = await this.model.findById(courseId).populate("instructor", "name")

    return course;
  }

  async findCoursesByInstructor(instructorId: string): Promise<ICourse[]> {
    return await this.model.find({instructor: instructorId }).sort({ createdAt: -1 });
  }

  async updateCourseStatus(courseId: string, isActive: boolean): Promise<ICourse | null> {
